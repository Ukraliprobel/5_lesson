...
def repeats(our_str):
    ...
    """
    функция принимает на вход строку, смотрит, сколько раз каждый символ уже встречался,
    и добавляет количество к символам с помощью постфикса формата _n. Возвращается новая строка.
    Например (Ввод --> Вывод):
    'letter' --> l_1e_1t_1t_2e_2_r_1
    :param our_str: Строка
    :return: Новая строка с нумерацией вхождений символов
    """
    list_characters_our_str = list(our_str)
    letters_dict = {}
    new_str = str()
    for i in list_characters_our_str:
        if i not in letters_dict:
            letters_dict.update({i: 1})
        else:
            letters_dict[i] += 1
        new_str = f"{new_str}{i}_{letters_dict[i]}"
    return new_str
