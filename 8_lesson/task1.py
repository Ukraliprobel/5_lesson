...
def treatment_sum(our_tuple):
    """
    Функция принимает кортеж our_tuple и возвращает результат сложения/обрабатывает исключение.
    """
    if len(our_tuple) > 2:
        raise Exception('Много данных')
    try:
        sum_two_elements = our_tuple[0] + our_tuple[1]
    except TypeError:
        return 'Нельзя сложить эти данные'
    except IndexError:
        return 'Недостаточно данных'
    return sum_two_elements
